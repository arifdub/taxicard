...
export default async function AdminDrivers({
  ...
  let query = supabase
    .from('profiles')
    .select('id, name, slug, phone, email, licence_number, is_admin, can_dispatch, is_pro, is_business, is_active, is_available, created_at')
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-2">
                <span
                  ...
                  aria-hidden="true"
                />
                {d.can_dispatch && !d.is_admin ? (
                  <span className="rounded-full bg-brandblue/20 px-2.5 py-1 text-xs font-bold text-brandblue">
                    office
                  </span>
                ) : null}
                {d.is_admin ? (
                  <span className="rounded-full bg-yellow px-2.5 py-1 text-xs font-bold text-navy">
                    admin
                  </span>
                ) : null}
                {d.is_business ? (
