...
export default async function CustomersPage({
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-white light:text-navy">Customers</h1>
      ...
      {customers.length === 0 ? (
        ...
        <ul className="space-y-3">
          {customers.map((c) => (
            <li
              key={c.id}
              className="rounded-2xl border border-white/10 light:border-slate-200 bg-navy-soft light:bg-white p-4"
            >
              <div className="flex items-start justify-between gap-3">
                <Link href={`/dashboard/customers/${c.id}`} className="min-w-0">
                  <p className="truncate text-base font-semibold text-white light:text-navy">
                    {c.name}
                  </p>
                  <p className="text-sm text-slate-400 light:text-slate-500">{c.phone}</p>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                <a
                  ...
                  className="rounded-xl border border-white/10 light:border-slate-200 px-3 py-2.5 text-center text-sm font-medium"
                >
                  Call
                </a>
                <a
                  ...
                  className="rounded-xl border border-white/10 light:border-slate-200 px-3 py-2.5 text-center text-sm font-medium text-emerald-300"
                >
                  WhatsApp
                </a>
              </div>
